...
const runFFmpegWorker = (
  ...
  meta: { dir: string; format: string }
): Promise<string> => {
  return new Promise((resolve, reject) => {
    console.log("hello8");

    const worker = new Worker(path.resolve("worker-dist/convert_worker.js"), {
    ...
    worker.on("message", (message: WorkerMessage) => {
      if (message.success && message.src) {
        ...
        reject(new Error(message.error ?? "Unknown error in FFmpeg worker"));
      }
    });
...
export async function POST(req: Request): Promise<Response> {
  ...
  try {
    ...
    const outputFormat =
      typeof format === "string" ? format.toLowerCase() : "mp4";
